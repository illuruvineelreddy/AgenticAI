...
class UpstoxBroker(BaseBroker):
    ...
    def __init__(self):
        ...
        if self.use_mock:
            logger.warning("Upstox API credentials missing. Falling back to MockBroker.")
    ...
    async def connect(self) -> bool:
        ...
        try:
            logger.info("Initializing real Upstox API v2 session")
            # In a real environment, initialize client session with access tokens
            return True
        except Exception as e:
            logger.error("Failed to connect to Upstox API, falling back to MockBroker", error=str(e))
            self.use_mock = True
            self.mock_broker = MockBroker()
            return await self.mock_broker.connect()
    ...
    async def disconnect(self) -> bool:
        ...
        logger.info("Disconnecting Upstox session")
        return True
