...
class FyersBroker(BaseBroker):
    ...
    def __init__(self):
        ...
        if self.use_mock:
            logger.warning("Fyers API credentials missing. Falling back to MockBroker.")
    ...
    async def connect(self) -> bool:
        ...
        try:
            logger.info("Initializing real Fyers API v3 session")
            # In a real environment, initialize client session with access tokens
            return True
        except Exception as e:
            logger.error("Failed to connect to Fyers API, falling back to MockBroker", error=str(e))
            self.use_mock = True
            self.mock_broker = MockBroker()
            return await self.mock_broker.connect()
    ...
    async def disconnect(self) -> bool:
        ...
        logger.info("Disconnecting Fyers session")
        return True
